...
def int_reward_func(completions, **kwargs) -> list[float]:
    responses = [completion[0]['content'] for completion in completions]
    extracted_responses = [extract_xml_answer(r) for r in responses]
    return [2 if r.isdigit() else -1 for r in extracted_responses]
...
def length_reward_func(prompts, completions, answer=None, trainer_state=None, **kwargs):
    import math

    tokenizer = kwargs.get('tokenizer')
    assert tokenizer is not None, "Tokenizer missing in kwargs!"
    ...
    reward = 0.0
    for r in responses:
        tokens = tokenizer(r).input_ids
        L = len(tokens)
        if L > 400:
            reward -= 1.0  
        else:
            exponent = -((L - mu) ** 2) / (2 * sigma ** 2)
